...
def load_gt(nusc: NuScenes, eval_split: str, box_cls, pred_sample_tokens, verbose: bool = False):
    """
    ...
    sample_tokens = []
    keyframe3_sample_tokens = []
    for sample_token in sample_tokens_all:
        scene_token = nusc.get('sample', sample_token)['scene_token']
        scene_record = nusc.get('scene', scene_token)
        if scene_record['name'] in splits[eval_split]:
            sample_tokens.append(sample_token)
            if index_map[sample_token] <= 3:
                keyframe3_sample_tokens.append(sample_token)
    if len(keyframe3_sample_tokens) != len(pred_sample_tokens):
        print(f"Your prediction length = {len(pred_sample_tokens)}, "
              f"3 keyframe sample token length = {len(keyframe3_sample_tokens)}, "
              f"which is not equal, please double check your code!")
    sample_tokens = list(pred_sample_tokens)
...
class NuScenesEval_custom(NuScenesEval):
    ...
    def __init__(self,
        ...
        self.gt_boxes = load_gt(
            ...
            self.pred_boxes.sample_tokens, verbose=verbose)
